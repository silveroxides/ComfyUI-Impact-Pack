...
class SEGSDetailerForAnimateDiff:
    ...
    def INPUT_TYPES(cls):
        return {"required": {
                     ...
                     "max_size": ("FLOAT", {"default": 768, "min": 64, "max": MAX_RESOLUTION, "step": 8, "tooltip": "Maximum size for a segment after scaling for detail enhancement. Prevents excessive upscaling."}),
                ...
                "optional": {
                     "refiner_basic_pipe_opt": ("BASIC_PIPE", {"tooltip": "Optional basic pipe for a refiner stage. Provides refiner model, CLIP, positive and negative conditioning."}),
                     "noise_mask_feather": ("INT", {"default": 20, "min": 0, "max": 100, "step": 1, "tooltip": "Feathering amount (in pixels) for the noise mask, if a noise mask is used."}),
                     "scheduler_func_opt": ("SCHEDULER_FUNC", {"tooltip": "Optional custom scheduler function to override the standard scheduler selection."}),
                     }
                }
...
class DetailerForEachPipeForAnimateDiff:
    ...
    def INPUT_TYPES(cls):
        return {"required": {
                      ...
                      "guide_size_for": ("BOOLEAN", {"default": True, "label_on": "bbox", "label_off": "crop_region", "tooltip": "Determines if 'guide_size' refers to the bounding box ('bbox') or the cropped region ('crop_region') for scaling."}),
                      "max_size": ("FLOAT", {"default": 1024, "min": 64, "max": nodes.MAX_RESOLUTION, "step": 8, "tooltip": "Maximum size for a segment after scaling for detail enhancement. Prevents excessive upscaling."}),
                      "seed": ("INT", {"default": 0, "min": 0, "max": 0xffffffffffffffff, "tooltip": "Seed for the random number generator used in the sampling process."}),
                      "steps": ("INT", {"default": 20, "min": 1, "max": 10000, "tooltip": "Number of sampling steps for the detail enhancement process."}),
                      "cfg": ("FLOAT", {"default": 8.0, "min": 0.0, "max": 100.0, "tooltip": "Classifier Free Guidance scale: higher values enforce prompt stronger."}),
                      "sampler_name": (comfy.samplers.KSampler.SAMPLERS,),
                ...
                "optional": {
                      "detailer_hook": ("DETAILER_HOOK", {"tooltip": "Optional hook for custom operations after each segment is pasted back to the image frames."}),
                      "refiner_basic_pipe_opt": ("BASIC_PIPE", {"tooltip": "Optional basic pipe for a refiner stage. Provides refiner model, CLIP, positive and negative conditioning."}),
                      "noise_mask_feather": ("INT", {"default": 20, "min": 0, "max": 100, "step": 1, "tooltip": "Feathering amount (in pixels) for the noise mask, if a noise mask is used in the detailing process."}),
                      "scheduler_func_opt": ("SCHEDULER_FUNC", {"tooltip": "Optional custom scheduler function to override the standard scheduler selection in the detailing process."}),
                      }
                }
